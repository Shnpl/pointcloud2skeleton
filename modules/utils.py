...
def accuracy(pred, target, thr=5):
    """
    ...
    target:BxJx3
    """
    batchsize,joint_num,_ = pred.shape
    pred = pred.cpu().detach().numpy()[:,:joint_num,:]
    target = target.cpu().detach().numpy()[:,:joint_num,:]
    dists = calc_dists(pred, target)# BxJ

    acc = np.zeros(batchsize)
    avg_acc = 0
    cnt = 0
...
def calc_dists(preds, target):
    # dists = np.zeros((preds.shape[0], preds.shape[1]))# BxJx3
    # for b in range(preds.shape[0]):
    #     for j in range(preds.shape[1]):
    dists = np.linalg.norm(preds - target,axis=2)# BxJ
